...
GLwindow_symbian::GLwindow_symbian(QWidget *parent)
    : QWidget(parent)
    , framecount(1)
    , fps(1.0)
    , window_w(1)
    ...
    , init(0)
{
    ...
    clear_color_b = 1.0f;
    clear_color_d = 0.0f;
    this->setWindowTitle("GLwindow_symbian 0.01");
}

GLwindow_symbian::~GLwindow_symbian()
{

}

float GLwindow_symbian::get_fps(){
    return fps;
}

float GLwindow_symbian::get_timestep(){
    return timestep;
}

int GLwindow_symbian::get_renderstep(){
    return renderstep;
}

int GLwindow_symbian::get_window_width(){
    return window_w;
}

int GLwindow_symbian::get_window_height(){
    return window_h;
}

float GLwindow_symbian::get_window_aspect(){
    return window_aspect;
}

void GLwindow_symbian::mouseReleaseEvent(QMouseEvent *event)
{
    mouseReleased(event);
}
...
{
    //do nothin, can be reimplemented
}

void GLwindow_symbian::mousePressEvent(QMouseEvent *event)
{
    mousePressed(event);
}
...
void GLwindow_symbian::mouseMoveEvent(QMouseEvent *event)
{
    mouseMoved(event);
}
...
bool GLwindow_symbian::initialized(){
    if(init == 1){
        return true;
    }else{
        return false;
...
void GLwindow_symbian::set_clear_color(float r, float g, float b, float d){
    clear_color_r = r;
    clear_color_g = g;
    clear_color_b = b;
    clear_color_d = d;
...
void GLwindow_symbian::resizeEvent(QResizeEvent *event){
...
void GLwindow_symbian::initEGL(){
    ...
    if (!eglInitialize(eglDisplay, &majorVersion, &minorVersion)) {
        qDebug("GLwindow_symbian: Error: initEGL() failed!!!");
        cleanupAndExit(eglDisplay);
    }

    qDebug("GLwindow_symbian: initEGL() finished");
    ...
    if (!eglChooseConfig(eglDisplay, pi32ConfigAttribs, &eglConfig, 1, &configs)
            || (configs != 1))  {
        qDebug("GLwindow_symbian: Error: eglChooseConfig() failed!");
        cleanupAndExit(eglDisplay);
    }

    qDebug("GLwindow_symbian: eglChooseConfig() finished");
    ...
    qDebug("GLwindow_symbian: eglCreateWindowSurface() finished");
    ...
    eglContext = eglCreateContext(eglDisplay, eglConfig, NULL,
                                  pi32ContextAttribs);
    qDebug("GLwindow_symbian: eglCreateContext() finished");
    ...
    qDebug("GLwindow_symbian: eglMakeCurrent() finished");

    if (!testEGLError("eglMakeCurrent")) {
        cleanupAndExit(eglDisplay);
    }
    qDebug("GLwindow_symbian: EGL initialization successfull...");
    init = 1;
}

bool GLwindow_symbian::testEGLError(const char *pszLocation)
{
...
void GLwindow_symbian::cleanupAndExit(EGLDisplay eglDisplay){
    eglMakeCurrent(eglDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
    eglTerminate(eglDisplay);
    exit(0);
}

void GLwindow_symbian::swapbuffer(){
